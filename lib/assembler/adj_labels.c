    ...
    for (uint32_t i = 0; i < assembly->functions_n; ++i) {
        ...
        for (uint32_t j = 0; j < assembly->functions[i].n_instructions; ++j) {
            ...
            if (inst->operation.type == OP_LABEL) {
                khiter_t k = kh_get_LBL(label_pc, inst->operation.v.label);
                if (k == kh_end(label_pc))
                    ERROR("Label '%s' not found", inst->operation.v.label);
                free(inst->operation.v.label);
                inst->operation.type = OP_INT;
                inst->operation.v.i = (int32_t) kh_value(label_pc, k);
            }
        }
