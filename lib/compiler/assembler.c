...
typedef enum { TA_DIRECTIVE, TA_NUMBER, TA_COLON, TA_REAL, TA_STRING, TA_INSTRUCTION } TokenType;
    ...
    for (const char* start = code, *p = code; ; ++p) {
        if (*p == '\n' || *p == '\0') {
            ...
            // if .func
                ...
                f_id = tokens[1].v.i;
                if (f_id < 0) ERROR("Function must be a positive number")
                assembly_add_function(as, f_id);
                section = TS_FUNC;
            }
            ...
            // constants
                ...
                if (tokens[0].v.i < 0) ERROR("Const must be a positive number")
                if (tokens[2].type == TA_NUMBER)
                    assembly_add_const_real(as, tokens[0].v.i, tokens[0].v.d);
                else
                    assembly_add_const_str(as, tokens[0].v.i, tokens[0].v.s);
            }
...
skip:
            for (size_t i = 0; i < n_tokens; ++i)
                if (tokens[i].type == TA_DIRECTIVE || tokens[i].type == TA_INSTRUCTION)
                    free(tokens[i].v.s);
            free(line);
            if (!*p) break;
            start = p + 1;
